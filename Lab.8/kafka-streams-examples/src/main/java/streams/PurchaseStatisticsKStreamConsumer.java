...
public class PurchaseStatisticsKStreamConsumer {
	...
	public static void main(final String[] args) throws Exception {
		...
		purchases
				.peek((key, value) -> LOG.info("Stateless Peek - key = {}, value = {}", key, value))
				.filter((key, value) -> value.getAmount() > 100) // Example filter condition
				.mapValues(value -> value.getAmount() * 2) // Example mapValues operation
				.map((key, value) -> new KeyValue<>(key, "MappedValue: " + value)); // Example map operation
		...
		purchases.groupBy((key, value) -> value.getProduct().toString(),
				...
				.count()
				.toStream()
				.peek((groupKey, value) -> LOG.info("Stateful Peek - groupKey = {}, value = {}", groupKey, value))
				.to("PurchaseStatistics", Produced.with(Serdes.String(), Serdes.Long()));

		// Additional stateful operations
		purchases.groupBy((key, value) -> String.valueOf(value.getCustomerId()),
						Grouped.with(Serdes.String(), specificAvroSerde))
				.reduce((value1, value2) ->  {value1.setAmount(value1.getAmount() + value2.getAmount()); return value1;})
				.toStream()
				.mapValues(value -> value.getAmount())
				.peek((groupKey, value) -> LOG.info("Reduced Value - groupKey = {}, value = {}", groupKey, value))
				.to("CustomerPurchaseTotal", Produced.with(Serdes.String(), Serdes.Long()));
